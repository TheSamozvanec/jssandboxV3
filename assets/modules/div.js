...
После импорта вы можете посмотреть текст функция, для этого используйте внутреннюю функцию printC(). Внутренняя функция print() не экранирует верстку и текст функции будет некорректным.
...
Начинающим программистам я рекомендую изучить этот момент! Дело в том, что формально импорт передает только импортированную функцию, но у функции своя область видимости, свой <b>name space</b>. Именно по этому объекты <b>monitor</b> и <b>style</b>, которые доступны в окне code, не видны в импортированной функции и их нужно явно передать, чтобы все работало, но функция работает без явных импортов input() и insert(), так как они в её поле видимости.

Все модули, которые я буду размещать будут иметь экспорт для любых функций, используемых внутри модуля. То есть Вы всегда сможете импортировать и взглянуть своими глазами, что за функция будет запущена. Это ваше спокойствие и безопасность, так вы будете уверены что не запускаете вредоносный код. Любой код, в котором вы засомневаетесь можно загуглить пропустить через AI или исследовать другим способом прежде чем запустить. Приятный бонус таких действий - если вы чего-то раньше не знали - теперь знаете &#128513;
`